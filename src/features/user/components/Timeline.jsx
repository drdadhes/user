...
const Timeline = () => {
  ...
  const milestones = [
    {
      icon: Feather,
      title: t?.timeline?.milestones?.[0]?.title || "Conscious Awakening",
      desc: t?.timeline?.milestones?.[0]?.desc || "Begin your journey with mindful awareness of your body's natural rhythms",
      color: "emerald",
      metrics: t?.timeline?.milestones?.[0]?.metrics || "82% report increased awareness",
      gradient: "from-emerald-400 to-emerald-600",
    },
    {
      icon: Activity,
      title: t?.timeline?.milestones?.[1]?.title || "Dosha Realignment",
      desc: t?.timeline?.milestones?.[1]?.desc || "Ancient pulse diagnosis reveals your unique constitution for personalized healing",
      color: "teal",
      metrics: t?.timeline?.milestones?.[1]?.metrics || "3 distinct dosha types",
      gradient: "from-teal-400 to-teal-600",
    },
    {
      icon: Brain,
      title: t?.timeline?.milestones?.[2]?.title || "Neural Harmony",
      desc: t?.timeline?.milestones?.[2]?.desc || "Modern neuroscience meets ancient wisdom for optimal mind-body connection",
      color: "indigo",
      metrics: t?.timeline?.milestones?.[2]?.metrics || "47% stress reduction",
      gradient: "from-indigo-400 to-indigo-600",
    },
    {
      icon: Heart,
      title: t?.timeline?.milestones?.[3]?.title || "Cellular Renewal",
      desc: t?.timeline?.milestones?.[3]?.desc || "Deep tissue restoration through specialized Ayurvedic protocols",
      color: "rose",
      metrics: t?.timeline?.milestones?.[3]?.metrics || "28-day transformation cycle",
      gradient: "from-rose-400 to-rose-600",
    },
    {
      icon: Zap,
      title: t?.timeline?.milestones?.[4]?.title || "Vitality Integration",
      desc: t?.timeline?.milestones?.[4]?.desc || "Sustainable wellness becomes your natural state of being",
      color: "amber",
      metrics: t?.timeline?.milestones?.[4]?.metrics || "365 days of lasting change",
      gradient: "from-amber-400 to-amber-600",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                <div className="flex items-center space-x-2 group">
                  ...
                  <span className="text-emerald-700 text-sm tracking-widest uppercase font-medium">
                    {t?.timeline?.badge || "The Path Within"}
                  </span>
                </div>
                <h2 className="text-5xl lg:text-6xl font-serif font-bold text-slate-900 leading-tight">
                  {t?.timeline?.heading?.line1 || "Your 5-Step"}
                  <span className="block text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600">
                    {t?.timeline?.heading?.line2 || "Transformation"}
                  </span>
                </h2>
              </div>

              <p className="text-lg text-slate-600 leading-relaxed">
                {t?.timeline?.subtitle || "Each phase of your wellness journey builds upon the last, creating lasting transformation that radiates from the inside out."}
              </p>

              {/* Progress indicator with glow */}
              <div className="space-y-4">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">{t?.timeline?.progressLabel || "Journey Progress"}</span>
                  <span className="text-emerald-700 font-medium relative">
                    {Math.round(((activeIndex + 1) / milestones.length) * 100)}%
                    <span className="absolute -inset-1 bg-emerald-400/20 rounded-full blur-md"></span>
                  </span>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center space-y-2 group">
          <span className="text-xs text-slate-400 tracking-widest group-hover:text-emerald-600 transition-colors">
            {t?.timeline?.scrollIndicator || "SCROLL TO EXPLORE"}
          </span>
          <div className="relative">
            <div className="absolute inset-0 bg-emerald-400 rounded-full blur-md opacity-0 group-hover:opacity-50 transition-opacity"></div>
            <div className="relative w-6 h-10 border-2 border-emerald-200 rounded-full flex justify-center">
